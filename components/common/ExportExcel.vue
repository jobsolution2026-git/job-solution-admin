...
const download = async () => {
  ...
  if (error && error.value) {
    ...
    link.href = url
    const filename = props.fileName ?? 'exported-data'
...
<template>
  <button @click="download" class="px-3 py-2 text-xs font-medium text-center text-white bg-yellow-400 rounded-lg hover:bg-yellow-500 focus:ring-4 focus:outline-none focus:ring-yellow-300 dark:bg-yellow-400 dark:hover:bg-yellow-400 dark:focus:ring-yellow-500">
