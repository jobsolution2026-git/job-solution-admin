...
const chapterOptions = computed(() => {
  if (subject.value) {
    const selectedSubjects = tags.value.filter(tag => subject.value.includes(tag.id))

    if (selectedSubjects.length) {
      SelectedChapter.value = selectedSubjects.map(subject => subject.tags).flat()
...
const topicOptions = computed(() => {
  if (chapter.value && SelectedChapter.value) {
    const selectedChapters = SelectedChapter.value.filter(tag => chapter.value.includes(tag.id))
    if (selectedChapters.length) {
      return selectedChapters.map(tag => tag.tags).flat().map(tag => ({label: tag?.name, value: tag?.id}))
    }
  }
})
...
const unitOptions = computed(() => {
  if (university.value) {
    const selectedUniversity = tags.value.filter(tag => university.value.includes(tag.id))
    if (selectedUniversity.length) {
      return selectedUniversity.map(tag => tag.tags).flat().map(tag => ({label: tag?.name, value: tag?.id}))
    }
  }
})
...
const searchMcq = handleSubmit(async values => {
  if (values && !Object.keys(values).length) {
    ...
    const {data, error} = await postData('admin/mcq/search-by-tag', {tag_ids: payload.flat()})
    if (error && error.value) {
...
<template>
  <section class="py-3 sm:py-5">
    <form v-if="showForm" class="px-4 mx-auto max-w-screen-2xl lg:px-12" @submit.prevent="searchMcq">
      <div class="my-2 grid grid-cols-2 gap-x-2">
        <div>
      ...
      <div class="grid grid-cols-2 gap-x-2 my-4">
        <div>
      ...
      <div class="mt-2 flex justify-center gap-2">
        <button type="submit"
          ...
          <svg v-if="isMcqLoading" aria-hidden="true" role="status" class="inline w-4 h-4 me-3 text-white animate-spin"
    ...
    <div class="mt-4">
      <div v-if="mcqs && mcqs.length" class="flex flex-wrap justify-between fixed bottom-2 right-2">
        <div>
          <button @click="selectAndUnselectAll"
                  class="mr-2 text-white bg-green-600 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-green-300 dark:focus:ring-green-800 font-medium rounded-lg text-xs inline-flex items-center px-3 py-2 text-center">
        ...
        <button @click="showForm = !showForm"
                class="ml-2 text-white bg-yellow-600 hover:bg-yellow-800 focus:ring-4 focus:outline-none focus:ring-yellow-300 dark:focus:ring-yellow-800 font-medium rounded-lg text-xs inline-flex items-center px-3 py-2 text-center">
          {{ showForm ? 'Hide Search' : 'Show Search' }}
        </button>
      </div>
      <div class="mt-4">
        <div v-if="mcqs && mcqs.length" class="grid gap-4 sm:grid-cols-2">
          <div v-for="(mcq, i) in mcqs" :key="mcq.id" @click="addIntoSelectedMcqIds(mcq.id)"
               :class="{ 'bg-primary-100': selectedMcqIds.includes(mcq.id) }"
