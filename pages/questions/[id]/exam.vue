...
const chapterOptions = computed(() => {
  if (subject.value) {
    ...
    if (selectedSubject) {
      ...
      return SelectedChapter.value.map(tag => ({ label: tag?.name, value: tag?.id })).sort((a, b) => a.label.localeCompare(b.label, 'en', { sensitivity: 'base' })) || [];
    }
  }
  return [];
});
      ...
      <div class="mt-4">
        <div v-if="mcqs && mcqs.length" class="grid gap-4 sm:grid-cols-2">
          <div v-for="(mcq, i) in mcqs" :key="mcq.id" @click="addIntoSelectedMcqIds(mcq.id)"
            :class="{ 'bg-primary-100': selectedMcqIds.includes(mcq.id) }" class="p-4 border rounded-lg cursor-pointer">
            <div>
