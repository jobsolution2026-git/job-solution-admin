...
const init = async () => {
  ...
  const {
    ...
    error,
    refresh
  } = await getData(`${pageInfo.value.apiUrl}?sectionable_type=${route.query.type}&sectionable_id=${route.query.id}`);
...
const dragEnd = async (event: any) => {
  ...
  const {
    ...
    error,
    refresh
  } = await postData('admin/arrange-order/Section', {items: items.value})
            ...
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
              ...
              <draggable v-if="!loader.isLoading &&  items.length" class="w-full" v-model="items" tag="tbody"
                ...
                <template #item="{ element }" class="w-full">
                  <tr class="border-b dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700" :key="element.id">
                    <th scope="row" class="flex items-center px-4 py-2 font-medium text-gray-900  dark:text-white">
                      ...
                      <nuxt-link :to="`/section/${element.id}?type=section`"
                                 class="font-medium text-blue-600 dark:text-blue-500 hover:underline">
                        {{ element.title }}
                      </nuxt-link>
                    </th>
                    <td class="px-4 py-2 mr-2 ">
                      <nuxt-link class="underline text-blue-500" :to="`/content/${element.id}`">Contents</nuxt-link>
                    </td>
