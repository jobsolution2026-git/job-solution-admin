...
import ContentViewModal from "~/components/common/FavouriteMcqViewModal.vue";
import FavouriteMcqViewModal from "~/components/common/FavouriteMcqViewModal.vue";
import FavouriteCqViewModal from "~/components/common/FavouriteCqViewModal.vue";

const pageInfo = ref<PageInfo>({
  title: 'Favourite CQS',
  description: 'Show all favourite CQ',
  apiUrl: '/admin/cq-user',
});
            ...
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
              ...
              <tr v-if="!loader.isLoading &&  items"
                ...
                <td class="px-4 py-2 mr-2">
                  <client-only>
                    <div v-katex="item?.cq?.question" class="font-bold text-black latex"></div>
                  </client-only>
                </td>
                <td class="px-4 py-2 mr-2">
                  <p class="font-medium text-black dark:text-white">{{ item?.user?.name }}</p>
                </td><td class="px-4 py-2 mr-2">
                <p class="font-medium text-black dark:text-white">{{ item?.user?.email }}</p>
              </td><td class="px-4 py-2 mr-2">
                <p class="font-medium text-black dark:text-white">{{ item?.user?.phone }}</p>
              </td>
                <td class="px-4 py-2 mr-2">
                  <FavouriteCqViewModal :item="item"/>
                </td>
              </tr>
              <tr v-else>
