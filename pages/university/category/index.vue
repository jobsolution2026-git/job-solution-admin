...
const pageInfo = ref<PageInfo>({
  ...
  description: 'Manage all your university categories',
  apiUrl: '/admin/university-categories',
});
...
const universityCategory = useUniversityCategoryStore();
...
if (universityCategory.categories && universityCategory.categories.length < 1) {
  universityCategory.fetchCategories()
}
//attributes
...
const {itemsPerPage,
  ...
  totalPages,
  paginatedItems,
  paginationLinks} = useTable(computed(() => universityCategory.categories), 'title')
//form
...
const onSubmit = handleSubmit(async values => {
  ...
  if (error && error.value) {
    ...
    if (editMode.value) {
      universityCategory.updateCategory(data.value.data);
    } else {
      universityCategory.addCategory(data.value.data);
    }
...
const deleteItem = async (event: number) => {
  selectedItem.value = universityCategory.items.find(item => item.id === event)
  ...
  if (error && error.value) {
    ...
    universityCategory.removeCategory(selectedItem.value.id);
