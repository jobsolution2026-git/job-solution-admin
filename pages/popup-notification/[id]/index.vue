...
const pageInfo = ref<PageInfo>({
  title: 'PopUp Notification Response',
  description: 'Manage all your PopUp Notification Response',
  apiUrl: '/admin/popup-notification-response',
});
...
const init = async (page: number = 1) => {
  ...
  let url = `${pageInfo.value.apiUrl}?popup_notification_id=${route.params.id}`;
            ...
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
              ...
              <tr v-if="!loader.isLoading &&  items"
                ...
                <td class="px-4 py-2 mr-2">
                  <p class="font-medium " :class="item?.response === 'rejected' ? 'text-red-500' : 'text-green-500'">{{ item?.response }}</p>
                </td>
                <td class="px-4 py-2 mr-2">
