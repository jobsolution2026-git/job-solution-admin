...
const copyAllNumbersToClipboard = () => {
  // Extract phone numbers from items
  const allPhoneNumbers = items.value
    .map((item) => item?.phone) 
    .filter((phone) => phone)   
    .join("\n");             
  ...
  // Copy to clipboard
  if (allPhoneNumbers) {
    navigator.clipboard.writeText(allPhoneNumbers).then(() => {
      showToast('success',"All phone numbers copied to clipboard!");
    }).catch((err) => {
            ...
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
              ...
              <tbody>
                ...
                <tr v-if="!loader.isLoading &&  items.length"
                  class="border-b dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700" v-for="item in items"
                  :key="item.id">
                  <th scope="row" class="flex items-center px-4 py-2 font-medium text-gray-900  dark:text-white">
                    {{ item?.name }}
                  </th>
                  <td class="px-4 py-2 mr-2 text-gray-900 dark:text-white cursor-pointer" @click="copyToClipboard(item?.phone)">
                  {{ item?.phone }}
                </td>
          ...
          <form @submit.prevent="onSubmit">
            <div class="grid gap-4 mb-4 sm:grid-cols-2">
              ...
              <div>
                ...
                <form-input-select v-model="role" v-bind="roleAttrs" :error="errors.role"
                  :options="[{label: 'Student', value: 'user'}, {label: 'Admin', value: 'admin'}, {label: 'Separator', value: 'separator'}]" />
              ...
              <div>
                ...
                <form-radio v-model="group" v-bind="groupAttrs" :error="errors.group"
